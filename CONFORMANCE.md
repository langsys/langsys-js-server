...
| **Core** | `langsys-js-typescript/pure` at **`239166a6`**, resolved through a `node_modules` symlink to the sibling checkout, whose tree is clean at that commit and whose `dist` is byte-identical to a clean build of a `git archive` of it. Mutation batteries run against that clean build |
...
| **Shared fixtures** | canonicalization-reference.json `ed6c3555` (32 rows); mig-vectors.json `20f2bdd6` (70 rows); snapshot-vectors.json `594bd77a` (8 rows, 4 refusals, 1 load); interpolation-reference.json `017bffdd` (25 rows); server-message-vectors.json `7333e391` (12 canonical entries, 13 markers, 10 fill, 10 resolve); custom-id-reference.json `633a09d9` (13 rows). Each is asserted by blob |
| **Suite** | 811 passing, 1 skipped, 28 files; 20 e2e tests (1 skipped by design); 4 runtimes × 26 checks with identity agreeing, the singleton-graph guard, and tarball acceptance with digest `1f284758e5ac3f6dbbb31a642252ca8c` — all measured in a git worktree whose core is a clean build of committed `239166a6` |
...
| MSG-1 | implemented | n/a (pure) | An entry is `template` and `params`, with `message` the fill, and the framework's own field and identifier passed through unchanged: a field keeps the framework's path format, a Pydantic `loc` array as an array. `langsys.attachMessages(body, entries, { key, pieces })` leaves the framework's error body exactly as it was and adds the entries under a configurable key (default `langsys_errors`, the Laravel package's), with configurable piece names; this package introduces no envelope. `msg`: all twelve canonical entries of `server-message-vectors.json` (`7333e391`), real Laravel, Pydantic, Django, DRF and ActiveModel messages, are rebuilt exactly by `message()`; every resolve row resolves to its entries with the body unchanged; and the four rows carrying an attached array of templated entries (Laravel 422, FastAPI 422, Rails with renamed pieces, an entry with no code) are rebuilt from the framework's native body by `message()` and `attachMessages()` and come back equal to the row's body, native members first. A Laravel-shaped and a NestJS-shaped body each resolve back through the key they were attached under; only template and params are required. Mutations: attach replacing the body 2 red; a non-string field dropped 2 red; piece names ignored 1 red; the default key changed 1 red. | all |
