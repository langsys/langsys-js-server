...
echo -e "${BLUE}🚀 langsys-js-server Publishing Script${NC}\n"
...
"
    while IFS= read -r commit; do
        if [ -n "$commit" ]; then
            ...
            COMMIT_MESSAGES="$COMMIT_MESSAGES- [\`$COMMIT_HASH\`](https://github.com/langsys/langsys-js-server/commit/$COMMIT_HASH)
  $FULL_COMMIT_MSG
...
\`\`\`bash
npm install langsys-js-server@$NEW_VERSION
\`\`\`$COMMIT_MESSAGES

## Full Changelog

See the [commit history](https://github.com/langsys/langsys-js-server/compare/v$ORIGINAL_VERSION...v$NEW_VERSION) for the complete diff."
...
log_info "Watch the run: https://github.com/langsys/langsys-js-server/actions"
log_success "Release page: https://github.com/langsys/langsys-js-server/releases/tag/v$NEW_VERSION"
log_success "npm (once CI finishes): https://www.npmjs.com/package/langsys-js-server/v/$NEW_VERSION"
